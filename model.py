...
class DCGAN(object):
	def __init__(self, sess, input_height=108, input_width=108, crop=True,
		...
		print(self.grayscale, self.c_dim)
		self.build_model()
	...
	def generator(self, z, y=None):
		with tf.variable_scope("generator") as scope:
			if not self.y_dim:
				...
				prevsize = s_h
				scale0 = None
				for ii in range(self.stacks + 1): # also generate for final layer
					ratio = 2 ** (ii + 1)
					prevsize = conv_out_size_same(prevsize, 2)
					lsz[ratio] = prevsize
					scale0 = ratio
				...
				self.h0 = tf.reshape(
						self.z_, [-1, lsz[scale0], lsz[scale0], self.gf_dim * 8])
